...
if __name__ == "__main__":
    ...
    for i in range(len(test_loader.dataset)):
        ...
        for per_class in list(class_map.keys()):
            if(per_class in vid_name[0]):
                class_map[per_class][0] += 1
                class_map[per_class][1] += res_item

    
    for per_class in list(class_map.keys()):
        if class_map [per_class][0] == 0:
            continue        
        else:
            class_map[per_class][2] = class_map[per_class][1] / class_map[per_class][0]
    
    
    
    frame_predict = np.concatenate(frame_predict)
    frame_predict_label = np.concatenate(frame_predict_label)
    metrics = get_metrics(frame_predict, frame_predict_label)
    AUC = metrics['AUC']
    AP = metrics['AP']
    print("AUC : ", AUC)
    print("AP : ", AP)
    for i in class_map:
        print(i, " /", class_map[i][0], "/ ", class_map[i][2])
