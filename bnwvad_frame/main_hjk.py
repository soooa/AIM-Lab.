...
if __name__ == "__main__":
    log_path = os.path.join(log_path, time_ymd, 'stack16_segment') # train log가 저장될 폴더 신경x
    model_path = os.path.join(model_path, time_ymd, 'stack16_segment') # 모델 저장될 폴더
...
########################################################################
        ...
        if step % 10 == 0: # 10 epoch 당 test 데이터에서 배출되는 score값을 조사한 후, 성능이 개선되면 가중치를 저장
            total_loss = test(test_loader, net)
